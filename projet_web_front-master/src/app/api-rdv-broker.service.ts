...
export class ApiRdvBrokerService {
  ...
  {
    this.httpClient.post<rdv>(this.url+'/'+idRDV+'/cloturer', rdv)
      .subscribe(
        (response)=>{console.log(response);}
        , (error)=>{console.log('Erreur Ajouter creneau')}
      )
  ...
  public ajouterParticip(rdv: rdv, id: number)
  {
    this.httpClient.post<rdv>(this.url+'profile/'+id+'/listeparticip/', rdv)
      .subscribe(
        (response)=>{console.log(response);}
        , (error)=>{console.log('Erreur Ajouter Liste Participant')}
      )
  }
}
